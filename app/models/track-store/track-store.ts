...
import { TrackSectionModel, TrackSectionSnapshot } from "../track-section/track-section"

export const TrackStoreModel = types
  .model("TrackStore")
  .props({
    classes: types.array(TrackSectionModel),
    poses: types.array(TrackSectionModel),
    music: types.array(TrackSectionModel),
  })
  .extend(withEnvironment)
  .actions((self) => ({
    saveClasses: (snapshots: TrackSectionSnapshot[]) => {
      self.classes.replace(snapshots)
    },
    savePoses: (snapshots: TrackSectionSnapshot[]) => {
      self.poses.replace(snapshots)
    },
    saveMusic: (snapshots: TrackSectionSnapshot[]) => {
      self.music.replace(snapshots)
    },
  }))
  .actions((self) => ({
    fetchClasses: () => {
